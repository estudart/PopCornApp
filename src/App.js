...
export default function App() {
  ...
  function handleDeleteMovie(id) {
    setWatched(watched.filter((movie) => movie.imdbIDg !== id));
    console.log(watched);
  }
...
function ErrorMessage({ message }) {
  return (
    <p className="error">
      {message === "Failed to fetch" ? "⛔" : "⛔ Movie not found"}
    </p>
  );
}
  ...
  useEffect(() => {
    function checkMovieList() {
      ...
      const foundMovie = watched.find(
        (watchedMovie) => watchedMovie.imdbIDg === selectedId
      );
      if (foundMovie) {
        setUserStarRating(foundMovie.userRating);
        setCheckWatched(true);
      }
    }
    console.log(watched);
    checkMovieList();
          ...
          <section>
            {checkWatched && (
              <div className="rating">
                {" "}
                You rated this movie ✨ {userStarRating}
              </div>
            )}
            {!checkWatched && (
              <StarRating
    ...
    <li key={movie.imdbID}>
      ...
      <div>
        ...
        <button
          ...
          onClick={() => onDeleteWatched(movie.imdbIDg)}
        >
          X
        </button>
      </div>
